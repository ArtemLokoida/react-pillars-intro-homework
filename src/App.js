...
const Population = ({ID_Nation, Nation, ID_Year, Year, Population, Slug_Nation}) =>
    <div>
        <p>ID Nation: {ID_Nation}</p>
        <p>Nation: {Nation}</p>
        <p>ID Year: {ID_Year}</p>
        <p>Year: {Year}</p>
        <p>Population: {Population}</p>
        <p>Slug Nation: {Slug_Nation}</p>
        <br/>
    </div>

const DisplayPopulation = () => populationData.map(item => 
    <Population
        ID_Nation = {item["ID Nation"]}
        Nation = {item["Nation"]}
...
const Swapi = ({name, height, mass, hair_color, skin_color, eye_color, birth_year, gender, homeworld, films, species, vehicles, starships, created, edited, url}) =>
    <div>
        <h3>Name: {name}</h3>
        <p>Height: {height}</p>
        <p>Mass: {mass}</p>
        ...
        {films.length > 0 && <p>Films: {films.map(item => <a href={item}><br/>{item}</a>)}</p>}
        {species.length > 0 && <p>Species: {species.map(item => <a href={item}><br/>{item}</a>)}</p>}
        {vehicles.length > 0 && <p>Vehicles: {vehicles.map(item => <a href={item}><br/>{item}</a>)}</p>}
        {starships.length > 0 && <p>Starships: {starships.map(item => <a href={item}><br/>{item}</a>)}</p>}
        <p>Created: {created}</p>
        <p>Edited: {edited}</p>
...
const DisplaySwapi = () => swapiData.map(item =>
    <Swapi
        name = {item.name}
        height={item.height}
        mass={item.mass}
...
const Episodes = ({name, air_date, characters}) =>
    <div>
        <h3>Name: {name}</h3>
        <p>Air date: {air_date}</p>
        <p>Characters: {characters.map((character, index) => <span>{character.name}{index === characters.length - 1 ? '.' : ', '}</span>)}</p>
        <br/>
    </div>

const DisplayEpisodes = () => episodesData.map(item => 
    <Episodes
    name = {item.name}
...
const Characters = ({name, gender, image, episodes}) =>
    <div>
        <h3>Name: {name}</h3>
        <p>Gender: {gender}</p>
        <div><img src={image}/></div>
        <span><p style={{fontWeight: "bold"}}>Episodes: </p>{episodes.map(episode => <p>Name: {episode.name}<br/>Air date: {episode.air_date}<br/></p>)}</span>
        <br/>
    </div>

const DisplayCharacters = () => charactersData.map(item => 
    <Characters
    name = {item.name}
    gender = {item.gender}
    image = {item.image}
